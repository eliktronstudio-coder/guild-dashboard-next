...
function buildPrompt(knownNames) {
  const knownNamesBlock =
    Array.isArray(knownNames) && knownNames.length > 0
      ? "\nСправочно, вот реальные ники состава гильдии (для сверки, а НЕ единственно " +
        "допустимый список — на скрине может быть гость, которого тут нет):\n" +
        knownNames.join(", ") +
        "\nЕсли прочитанный тобой ник визуально похож на один из этого списка (отличается " +
        "буквой, которую легко спутать) — используй точное написание из списка. Если ник " +
        "явно отличается или его нет в списке — впиши как прочитал, не подгоняй силой.\n"
      : "";
  ...
  return (
    ...
    "Читай ники МАКСИМАЛЬНО ТОЧНО, посимвольно, прежде чем записать — не исправляй кажущиеся " +
    "опечатки, не сокращай и не дополняй, сохраняй регистр букв и любые цифры/спецсимволы как есть. " +
    "Особо аккуратно различай визуально похожие символы: латиница/кириллица (a/а, e/е, o/о, p/р, c/с, " +
    "x/х, y/у), 0 (ноль) и О/O (буква), 1 (единица) и l/I (латинские) и л (кириллица), rn и m, " +
    "заглавные и строчные там, где мелкий шрифт их не различает явно — в таком случае бери тот " +
    "вариант регистра, что чаще встречается в подобных никах. Если часть ника не читается — не " +
    "додумывай, пропусти этот ник целиком, чем угадывать наугад." +
    knownNamesBlock +
...
app.post("/extract", async (req, res) => {
  ...
  const knownNames = Array.isArray(req.body?.knownNames)
    ? req.body.knownNames.filter((n) => typeof n === "string" && n.trim()).map((n) => n.trim()).slice(0, 300)
    : [];

  try {
    const parsed = await askVision(body.image, body.mediaType, buildPrompt(knownNames));
